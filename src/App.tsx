...
export default function App() {
  ...
  // Dữ liệu xã Phú Hòa từ Firestore (thay thế COMMUNE_DATA['X. Phú Hòa'])
  const currentZoneData = useMemo(() => {
    return communeData['X. Phú Hòa'] || EMPTY_COMMUNE_ENTRY;
  }, [communeData]);
  ...
  const [aiChatHistory, setAiChatHistory] = useState([
    {
      role: 'assistant',
      text: 'Xin chào Ban chỉ đạo Xã Phú Hòa! Tôi là Trợ lý AI Điều Hành DECC. Tôi đã đối soát hoàn chỉnh cả 2 Tầng dữ liệu: Tầng 1 (Chuẩn Phường/Xã 2.0 - DEI 76%) và Tầng 2 (Hệ sinh thái Kinh tế số). Sẵn sàng đồng hành cùng lãnh đạo để phân tích mâu thuẫn hệ thống và ra quyết định chính xác nhất.',
      timestamp: '07:41',
    },
  ]);
      ...
      contents: [
        {
          parts: [
            {
              text: 'Soạn một quyết định hành chính ngắn gọn để khắc phục điểm nghẽn đào tạo chuyển đổi số tại Xã Phú Hòa.',
            },
          ],
        },
      ],
  ...
  return (
    <div className="min-h-screen bg-[#07111F] text-white font-sans antialiased selection:bg-cyan-500 flex flex-col overflow-x-hidden">
                ...
                <div className="bg-[#0A2540] border border-cyan-500/15 rounded-3xl p-5 flex flex-col">
                  <div className="flex items-center gap-2 mb-5">
                    ...
                    <span className="text-sm font-bold text-cyan-400 uppercase tracking-wide">
                      B1: THÔNG TIN ĐƠN VỊ KINH DOANH - XÃ PHÚ HÒA
                    </span>
                  </div>
      ...
      <footer className="bg-[#0A2540] border-t border-cyan-500/10 px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4 mt-auto text-xs text-slate-400">
        ...
        <div>
          <span>© 2026 Ban Chỉ Đạo Chuyển Đổi Số Xã Phú Hòa.</span>
        </div>
      </footer>
    </div>
  );
